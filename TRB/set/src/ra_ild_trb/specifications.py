...
* selection of static TRB candidate predictors from the step-04 manifest;
...
STATIC_FEATURE_GROUP = "static_tcr_candidate_predictors"
...
# Backward-compatible aliases used by older unit tests and callers.
...
def select_static_tcr_features(
    manifest: pd.DataFrame,
    *,
    available_columns: Optional[Iterable[str]] = None,
    require_all_available: bool = False,
) -> List[str]:
    """Reproduce the V1 static-TRB feature-manifest selection rule.
    ...
    * ``reference_drop`` is false.
    ...
    if not selected:
        raise SpecificationError("No static TRB candidate predictors were selected.")
    return selected
