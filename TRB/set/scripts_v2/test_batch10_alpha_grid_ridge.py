#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Focused tests for Batch 10 alpha-grid overrides and alpha endpoints."""
...
from ra_ild_trb.config import validate_experiment_mapping
from ra_ild_trb.modeling import fit_elastic_net
from ra_ild_trb.specifications import build_hyperparameter_grid
...
def load_preparer_module():
    ...
    spec = importlib.util.spec_from_file_location("batch10_preparer", path)
...
def test_scheme_override_and_count_recalculation() -> None:
    ...
    test_id = "batch10_alpha_grid_test_tmp"
    source["scheme"]["id"] = test_id
    source["scheme"]["output_root"] = f"TRB/set/experiments/{test_id}"

    temporary_scheme = (
        REPOSITORY_ROOT
        / "TRB/set/configs/schemes/_batch10_alpha_grid_test_tmp.yaml"
    )
    output_root = REPOSITORY_ROOT / "TRB/set/experiments" / test_id
    try:
...
def main() -> int:
    ...
    print(f"Batch 10 focused tests: {passed}/{len(tests)} passed")
    return 0
