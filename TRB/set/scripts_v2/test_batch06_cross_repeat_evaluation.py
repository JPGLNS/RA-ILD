#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Focused tests for TRB frozen-model cross-repeat evaluation."""
...
def test_expected_counts() -> None:
    one = expected_cross_repeat_counts(3, 1, 51)
    ...
    assert one["prediction_rows"] == 459

    all_models = expected_cross_repeat_counts(3, 3, 51)
    assert all_models["model_instances"] == 9
    assert all_models["full_metric_rows"] == 27
    assert all_models["prediction_rows"] == 1377
...
def test_hybrid_public_splits_seen_and_unseen_rows() -> None:
    ...
    def fake_external(
        presence, frequency, reference_rows, target_rows, labels,
        aa_clone_numbers, scheme, epsilon, *, context
    ):
    ...
    with patch(
        "ra_ild_trb.cross_repeat_evaluation.external_public_features",
...
def main() -> int:
    ...
    print(f"Batch 06 focused tests: {len(tests)}/{len(tests)} PASS")
    return 0
