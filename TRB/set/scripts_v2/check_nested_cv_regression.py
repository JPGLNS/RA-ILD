...
from ra_ild_trb.public_reference import (  # noqa: E402
    ALL_PUBLIC_FEATURES,
    ThresholdScheme,
)
...
from ra_ild_trb.thresholds import choose_threshold_youden  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Compare the TRB V2 nested-CV engine with frozen V1 outputs.",
...
def build_selected(v1_tuning: pd.DataFrame, v1_oof: pd.DataFrame, model_names: Sequence[str]) -> Tuple[Mapping[str, SelectedTuning], List[Tuple[str, bool, str]]]:
    ...
    for model in model_names:
        ...
        from ra_ild_trb.specifications import HyperparameterCandidate
...
def main() -> int:
    ...
    try:
        ...
        add(results, "Outer task sample counts", split.n_outer_train == 98 and split.n_outer_valid == 25, f"train={split.n_outer_train}, validation={split.n_outer_valid}")
        ...
        static_features = select_static_tcr_features(manifest, available_columns=base.columns, require_all_available=True)
