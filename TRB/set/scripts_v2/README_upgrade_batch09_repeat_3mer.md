# TRB Batch 09：repeat级3-mer与静态feature分组升级
...
2. **将原1083个静态预测变量拆成可组合feature组**
   ...
   - 保留`static_tcr_candidate_predictors`兼容别名，含本repeat的83+500+500。
...
- `TRB/set/src/ra_ild_trb/repeat_3mer_features.py`
- `TRB/set/scripts_v2/prepare_feature_ablation_scheme.py`
- `TRB/set/scripts_v2/test_batch09_repeat_3mer_feature_groups.py`
- `TRB/set/configs/schemes/trb_scheme_004_feature_ablation_repeat3.yaml`
- `TRB/set/scripts_v2/BATCH09_REPEAT_3MER_FEATURE_GROUPS_MANIFEST.json`
...
- 复制新增/替换文件；
- 运行`py_compile`；
- 运行Batch 09 focused tests。

## 5. 准备feature消融方案

```bash
python TRB/set/scripts_v2/prepare_feature_ablation_scheme.py \
  --scheme TRB/set/configs/schemes/trb_scheme_004_feature_ablation_repeat3.yaml
```
...
```text
TRB/set/experiments/
trb_scheme_004_feature_ablation_repeat3/
...
python TRB/set/scripts_v2/run_all_outer_tasks.py \
  --config TRB/set/experiments/trb_scheme_004_feature_ablation_repeat3/00_config/resolved_config.yaml \
  --execute
```
...
```text
TRB/set/scripts_v2/run_nested_cv_task.py.bak.batch09.<时间戳>
```
