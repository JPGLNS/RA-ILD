#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run one TRB V2 nested-CV outer task into a separate V2 output directory.

Batch 09 adds optional outer-repeat-specific 3-mer vocabulary fitting and named
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one leakage-controlled TRB V2 nested-CV outer task.",
...
def main() -> int:
    ...
    try:
        ...
        print("TRB V2 nested-CV task: COMPLETE")
        ...
        print(
            "Tuning selection: "
            f"{options.tuning_primary_metric} "
            f"({next(iter(result.selected.values())).candidate_selection_policy})"
        )
        print(
        ...
        print(f"TRB V2 nested-CV task: FAIL\n{exc}", file=sys.stderr)
        return 1
