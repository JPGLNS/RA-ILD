#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Backfill log-loss and Brier score into completed repeated-holdout tasks.

This utility does not refit models. It deterministically recomputes all saved
holdout metrics from each task's frozen predictions, verifies the pre-existing
metrics, backs up the old metric CSV, and then adds ``log_loss`` and
``brier_score``. Afterward, rerun the repeated-holdout aggregator with
``--overwrite``.
"""
...
from ra_ild_trb.metrics import classification_metrics  # noqa: E402
from ra_ild_trb.outer_cv import build_outer_tasks  # noqa: E402
...
BACKUP_FILE = "06_outer_validation_metrics.before_batch13.csv"
...
class BackfillError(ValueError):
    """Raised when saved task outputs are inconsistent or unsafe to update."""
...
def _reordered_columns(frame: pd.DataFrame) -> List[str]:
    columns = [column for column in frame.columns if column not in NEW_METRICS]
...
def _recompute_task(task, *, tolerance: float, dry_run: bool) -> Dict[str, object]:
    ...
    _require_columns(
        metrics,
        ("model", "threshold", *EXISTING_METRICS),
    ...
    for row_index, model in enumerate(model_order):
        ...
        if group.empty:
            raise BackfillError(f"Task {task.task_id}/{model} has no predictions")
        ...
        if not np.isclose(float(thresholds[0]), saved_threshold, rtol=0.0, atol=tolerance):
    ...
    return {
        ...
        "backup_path": str(backup_path) if not already_current else "",
    }
...
def main() -> int:
    ...
    try:
        ...
        if len(tasks) != expected:
            raise BackfillError(
                f"Configured executed task count={len(tasks)}, expected split_count={expected}"
            )
        ...
        print("TRB probability-metric backfill: COMPLETE")
        ...
        if args.dry_run:
            ...
            print(
                "Next: rerun aggregate_repeated_holdout_results.py with --overwrite"
            )
        return 0
    except Exception as exc:
        print(f"TRB probability-metric backfill: FAIL\n{exc}", file=sys.stderr)
        return 1
