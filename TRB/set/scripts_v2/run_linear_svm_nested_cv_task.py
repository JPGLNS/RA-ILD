#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run one leakage-controlled TRB Linear SVM outer task."""
...
from ra_ild_trb.linear_svm_config import (  # noqa: E402
    candidate_summary,
    expand_linear_svm_candidates,
    load_linear_svm_config,
)
from ra_ild_trb.linear_svm_nested_cv import (  # noqa: E402
    LinearSVMNestedCVOptions,
    run_linear_svm_outer_task,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run one TRB Linear SVM nested-CV outer task.",
...
def enforce_output_policy(paths: Mapping[str, Path], overwrite: bool) -> None:
    ...
    if existing and not overwrite:
        raise FileExistsError(
            "Linear SVM outputs already exist; use --overwrite only for a documented rerun:\n"
            + "\n".join(f"  - {path}" for path in existing)
        )


def _legacy_feature_groups(
    static_features: Sequence[str], *, group_name: str
    ...
    core, unweighted, weighted = split_legacy_static_features(
        ...
        expected_core_count=len(
            [
                ...
                if not str(name).startswith(("unweighted_3mer_", "weighted_3mer_"))
            ]
        ),
    )
    ...
    for k in (50, 100, 200, 500):
        if k <= max_k:
            ...
            groups[f"repeat_3mer_both_top{k}"] = tuple(unweighted[:k]) + tuple(
                weighted[:k]
            )
    return groups
...
def _validate_expected_columns(config, model_specs) -> None:
    expected = config.raw.get("model_selection", {}).get(
        "expected_resolved_columns", {}
    )
...
def _write_static_union(path: Path, groups: Mapping[str, Sequence[str]]) -> None:
    ...
    pd.DataFrame(
        {"feature_order": np.arange(1, len(ordered) + 1), "feature_name": ordered}
...
def main() -> int:
    ...
    try:
        ...
        legacy_static_features = select_static_tcr_features(
            manifest, available_columns=base.columns, require_all_available=True
        )
        ...
        if repeat_options is not None:
            ...
            static_feature_groups = _legacy_feature_groups(
                legacy_static_features, group_name=legacy_group_name
            )
            rows = []
        ...
        options = LinearSVMNestedCVOptions(
            ...
            coefficient_nonzero_tolerance=float(
                modeling.get("coefficient_nonzero_tolerance", 1.0e-12)
            ),
            fit_intercept=bool(engine.get("fit_intercept", True)),
            intercept_scaling=float(engine.get("intercept_scaling", 1.0)),
        ...
        result = run_linear_svm_outer_task(
            base,
            outer,
            inner,
            row_lookup,
        ...
        result.public_reference_audit.to_csv(
            paths["public_reference_summary"], index=False
        )
        result.public_loo_assignments.to_csv(
        ...
        selected = {
            model: {
                **value.candidate.as_dict(),
                ...
                "youden_j": value.youden_j,
                "inner_roc_auc": value.inner_roc_auc,
                "inner_average_precision": value.inner_average_precision,
        ...
        print("TRB Linear SVM nested-CV task: COMPLETE")
        ...
        print(f"TRB Linear SVM nested-CV task: FAIL\n{exc}", file=sys.stderr)
        return 1
