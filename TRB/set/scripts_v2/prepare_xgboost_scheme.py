#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Prepare an isolated Batch 18 TRB XGBoost scheme and freeze its candidate bank."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Prepare a TRB Batch 18 XGBoost scheme.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
...
def prepare(source_path: Path, repository_root: Path, *, overwrite: bool, dry_run: bool) -> Path:
    ...
    if not isinstance(experiment, MutableMapping) or str(experiment.get("receptor")) != "TRB":
        raise XGBoostSchemeError("Base config must contain experiment.receptor=TRB")
    ...
    outer_tasks["runner_script"] = "TRB/set/scripts_v2/run_xgboost_nested_cv_task.py"
    ...
    if dry_run:
        print("RA-ILD TRB XGBoost scheme preparation")
