#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Focused synthetic acceptance test for TRB Batch 15 Linear SVM."""
...
from ra_ild_trb.linear_svm import (  # noqa: E402
    LinearSVMCandidate,
...
from ra_ild_trb.linear_svm_summary import write_aggregation  # noqa: E402
...
def _assert_baseline_import_compatibility() -> None:
    baseline = SRC / "ra_ild_trb/nested_cv.py"
    if baseline.is_file():
        module = importlib.import_module("ra_ild_trb.linear_svm_nested_cv")
        assert hasattr(module, "run_linear_svm_outer_task")
        runner = ROOT / "TRB/set/scripts_v2/run_linear_svm_nested_cv_task.py"
        assert runner.is_file()
    print("PASS baseline TRB V2 import compatibility")
...
def _test_scheme_preparation_and_status(temp_root: Path) -> None:
    ...
    base = {
        ...
        "model_selection": {"static_feature_group": "static_tcr_candidate_predictors"},
    ...
    preparer = ROOT / "TRB/set/scripts_v2/prepare_linear_svm_scheme.py"
    subprocess.run(
        [
            sys.executable,
            str(preparer),
    ...
    assert set(resolved["models"]) == {"M0_clinical", "M2_static_tcr_public"}
    orchestrator = ROOT / "TRB/set/scripts_v2/run_all_linear_svm_tasks.py"
    subprocess.run(
        [
            sys.executable,
            str(orchestrator),
...
def _test_aggregation_and_visualization(temp_root: Path) -> None:
    ...
    for repeat in range(1, 4):
        _write_task(
            ...
            repeat,
            {"M0_clinical": -0.05, "M2_static_tcr_public": 0.05},
        )
    ...
    assert set(metrics["model"]) == {"M0_clinical", "M2_static_tcr_public"}
...
def _test_probability_metrics_are_absent() -> None:
    source = (SRC / "ra_ild_trb/linear_svm_nested_cv.py").read_text(encoding="utf-8")
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="trb_batch15_acceptance_") as temp:
        temp_root = Path(temp)
    ...
    print("TRB_BATCH15_LINEAR_SVM_ACCEPTANCE_PASS")
    return 0
