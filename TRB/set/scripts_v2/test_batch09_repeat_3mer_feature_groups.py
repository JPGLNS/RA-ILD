#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Focused regression tests for Batch 09 repeat-specific 3-mer features."""
...
def test_outer_repeat_vocabulary_and_nested_top_k_groups() -> None:
    with tempfile.TemporaryDirectory(prefix="trb_batch09_") as temp:
        root = Path(temp)
        sample_sequences = {
        ...
        for sample_id, sequences in sample_sequences.items():
            path = root / f"{sample_id}_TRB_CDR3_AA_clone_table.csv"
        ...
        assert len(build.static_feature_groups["static_tcr_candidate_predictors"]) == 87
        assert not any(
...
def main() -> int:
    ...
    print(f"Batch 09 focused tests: {len(tests)}/{len(tests)} passed")
    return 0
