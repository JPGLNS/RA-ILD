...
Project model:
    ...
    age + sex + static TCR + dynamic reference-public features

Default workflow:
1. Read ONLY the 123-sample training base matrix.
2. Reuse the leakage-controlled LOO/public/preprocessing functions from
   run_05_single_outer_trial_loo.py.
3. Use 20 repeated stratified 5-fold splits over all 123 training samples.
...
6. Average each sample's 20 repeated OOF probabilities and lock one threshold
   without treating 2,460 repeated predictions as independent observations.
7. Generate full-training LOO public features (each sample uses the other 122),
   fit the final M2 on all 123 training samples, and save a deployable bundle.
8. The independent 51-sample test set is NEVER read.
"""
...
MODEL_NAME = "M2_static_tcr_public"
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=parse_candidate_pairs("0.9:10,0.5:30"),
    )
    parser.add_argument(
        "--l1-ratios",
...
def load_worker(path: Path) -> ModuleType:
    ...
    required = [
        "PUBLIC_FEATURE_SETS",
        "static_tcr_features_from_manifest",
...
def write_summary(
    ...
    lines = [
        ...
        "## Leakage boundary",
        "",
        "- Only the 123-sample training set was read.",
        "- The independent 51-sample test set was not read.",
        ...
        "- Final fitting public features used all other 122 training samples for each sample.",
        "",
        "## Candidate tuning",
        "",
...
def main() -> int:
    ...
    static_features = worker.static_tcr_features_from_manifest(manifest, base)
    ...
    print("=" * 80, flush=True)
    print("07 Final Full-Training M2 Tuning and Fit", flush=True)
    print("=" * 80, flush=True)
    print(f"Training samples: {len(base)}", flush=True)
    print(f"Static TCR predictors: {len(static_features)}", flush=True)
    ...
    if not (sample_mean_oof["n_repeated_predictions"] == len(args.repeats)).all():
        raise RuntimeError("Each sample does not have one selected-candidate OOF prediction per repeat.")
    ...
    final_model, final_converged, final_iterations = worker.fit_elastic_net(
        ...
        args=args,
        seed=derive_seed(args.seed, 799, 123),
    )
    ...
    # Full 123-sample public reference for future test/new-sample application.
    ...
    model_bundle = {
        ...
        "model_definition": "age + sex + static TCR + dynamic public",
        ...
        "static_tcr_features": list(static_features),
        ...
        "training_public_generation": "exact leave-one-out, n_reference=122",
        "new_sample_public_generation": "complete 123-sample training reference",
    ...
    print("", flush=True)
    print("[Threshold candidates on 123 sample-mean repeated OOF probabilities]", flush=True)
