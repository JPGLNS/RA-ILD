...
"""
Build final step-03 public CDR3-AA files after threshold stability evaluation.
...
Important
---------
The fixed full-training descriptive features generated in train mode are NOT
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build final step-03 public CDR3-AA files.",
...
def write_train_summary(
    ...
    lines = [
        "# 03 Public CDR3-AA Build Summary — TRAIN",
        "",
        "## Scope",
        "",
...
def write_test_summary(
    ...
    lines = [
        "# 03 Public CDR3-AA Build Summary — TEST",
        "",
        "## Scope",
        "",
...
def run_train(args: argparse.Namespace) -> int:
    ...
    print("=" * 80)
    print("03 Public CDR3-AA 正式构建 — TRAIN")
    print("=" * 80)
...
def run_test(args: argparse.Namespace) -> int:
    ...
    if definition.get("scheme", {}).get("name") != args.scheme_name:
        raise RuntimeError(
            f"reference scheme={definition.get('scheme', {}).get('name')}，"
            f"但命令要求scheme={args.scheme_name}。"
        )

    bit_mapping, set_sizes = load_reference_mapping(reference_file, definition)
    print("=" * 80)
    print("03 Public CDR3-AA 正式构建 — TEST")
    print("=" * 80)
