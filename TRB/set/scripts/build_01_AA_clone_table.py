...
"""
01_AA_clone_table: translate corrected TRB CDR3 nucleotide sequences and
aggregate synonymous NT clones into sample-level amino-acid clone tables.

Designed for files such as:
    {sample_id}_TRB_CDR3_NT_frequency_error_correct.csv
...
The script performs a full validation pass even in --dry-run mode and prints a
Chinese QC report including valid/invalid NT-row ratios, valid-read ratios,
invalid translation reasons, normalization checks, and low-quality samples.
"""
...
INPUT_SUFFIX = "_TRB_CDR3_NT_frequency_error_correct.csv"
OUTPUT_SUFFIX = "_TRB_CDR3_AA_clone_table.csv"
...
class InvalidDetailWriter:
    """Optional compressed detailed log with a configurable row limit."""
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "构建样本级 TRB CDR3 AA clone table，并输出完整翻译与归一化 QC。"
        ),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )

    parser.add_argument("--input-dir", required=True, help="NT clone 文件目录。")
    ...
    parser.add_argument(
        ...
        type=float,
        default=0.70,
        help="有效翻译 reads 比例低于该值时给出 warning。",
    )
    parser.add_argument(
        "--min-valid-nt-row-ratio",
        type=float,
        default=0.70,
        help="有效翻译 NT 输入行比例低于该值时给出 warning。",
    )
    parser.add_argument(
        "--min-nt-clones-warn",
        type=int,
        default=10,
        help="原始 NT 输入行少于该值时给出 warning。",
    )
    parser.add_argument(
        "--norm-tolerance",
        type=float,
        default=1e-8,
        help="frequency_norm 和 read_fraction 求和检查容差。",
    )
    parser.add_argument(
        "--top-n-low-quality",
        type=int,
        default=5,
        help="终端报告中列出的最低质量样本数量。",
    )
    parser.add_argument(
        "--invalid-example-per-reason",
        type=int,
        default=5,
        help="正式运行时每类无效原因保存的示例数；0 表示不保存。",
    )
    parser.add_argument(
        "--write-invalid-log",
        action="store_true",
        help="额外写出逐行无效序列明细压缩日志；大数据集默认不建议开启。",
    )
    parser.add_argument(
        "--invalid-log-max-rows",
        type=int,
        default=100000,
        help="详细无效日志最大行数；0 表示不限制，可能生成很大的文件。",
    )
    parser.add_argument(
        "--invalid-log-file",
        default=None,
...
def read_metadata_ids(metadata_path: Path, id_col: str) -> List[str]:
    with metadata_path.open("r", encoding="utf-8-sig", newline="") as handle:
        ...
        if duplicates:
            raise ValueError(
                "metadata 存在重复样本 ID: " + ", ".join(sorted(duplicates)[:10])
            )
        if not sample_ids:
...
def invalid_detail_row(
    ...
    fields_padded = list(fields_in[:4]) + [""] * max(0, 4 - len(fields_in))
    return {
        "sample_id": sample_id,
        "line_number": line_number,
        "cdr3_nt": fields_padded[0],
        "read_count_raw": fields_padded[1],
        "input_frequency_raw": fields_padded[2],
        "input_cell_frequency_raw": fields_padded[3],
        "reason": reason,
    }
...
def choose_frequency_source(
    ...
    sums = {
        "input_cell_frequency_sum": sum(
            float(row["input_cell_frequency_sum"]) for row in rows
        ),
...
def build_one_sample(
    ...
    def record_invalid(
        ...
        if args.invalid_policy == "error":
            raise ValueError(
                f"{input_path} 第 {line_number} 行无效: {reason}"
            )
    ...
    with input_path.open("r", encoding="utf-8-sig", errors="replace") as handle:
        for line_number, raw_line in enumerate(handle, start=1):
            ...
            if len(row_fields) < 4:
                ...
                if len(row_fields) >= 2:
                    try:
                        ...
                        read_count = None
                record_invalid(
                    "malformed_columns", line_number, row_fields, read_count
                )
                continue
            ...
            try:
                ...
                record_invalid(
                    "invalid_read_count", line_number, row_fields, None
                )
                continue
            ...
            try:
                ...
                record_invalid(
                    "invalid_frequency", line_number, row_fields, read_count
                )
                continue
            ...
            aa_record = aa_records.get(aa)
            if aa_record is None:
                aa_record = AARecord()
                aa_records[aa] = aa_record

            nt_record = aa_record.nt_records.get(nt)
            if nt_record is None:
                nt_record = NTRecord()
                aa_record.nt_records[nt] = nt_record
    ...
    stats.valid_read_ratio = safe_ratio(
        stats.total_reads_valid_nt, stats.total_reads_all_nt
    )
    stats.unique_valid_nt_clones = sum(
        len(record.nt_records) for record in aa_records.values()
    )
    stats.aa_clone_number = len(aa_records)
    stats.aa_clone_reduction_ratio = safe_ratio(
        stats.aa_clone_number, stats.unique_valid_nt_clones
    )
...
def write_aa_table(path: Path, rows: Sequence[Dict[str, object]]) -> None:
    ...
    write_csv_atomic(path, OUTPUT_COLUMNS, formatted_rows)
...
def print_configuration(
    ...
    print_rule("01_AA_clone_table：运行配置")
    ...
    print(f"有效 reads warning 阈值: {args.min_valid_read_ratio:.2f}")
    print(f"有效 NT 行 warning 阈值: {args.min_valid_nt_row_ratio:.2f}")
    print(f"dry-run: {'是（完整计算QC，但不写文件）' if args.dry_run else '否'}")
    print(f"覆盖旧文件: {'是' if args.overwrite else '否'}")
    print(f"逐行无效明细日志: {'开启' if args.write_invalid_log and not args.dry_run else '关闭'}")
...
def print_progress(index: int, total: int, stats: SampleStats) -> None:
    ...
    print(
        ...
        f"无效={format_int(stats.invalid_nt_rows)} | "
        f"{marker} | {stats.status}"
    )
...
def print_final_report(
    ...
    print(
        f"- 有效翻译 NT 行: {format_int(total_valid_rows)} "
        f"({format_pct(safe_ratio(total_valid_rows, total_input_rows))})"
    )
    print(
        f"- 无效 NT 行: {format_int(total_invalid_rows)} "
        f"({format_pct(safe_ratio(total_invalid_rows, total_input_rows))})"
    )
    print(f"- 全部可解析 reads: {format_int(total_reads)}")
    print(
        f"- 有效翻译 reads: {format_int(total_valid_reads)} "
        f"({format_pct(safe_ratio(total_valid_reads, total_reads), 4)})"
    )
    print(
        f"- 无效序列对应 reads: {format_int(total_invalid_reads)} "
        f"({format_pct(safe_ratio(total_invalid_reads, total_reads), 4)})"
    )
    unparseable_rows = sum(item.invalid_rows_without_parseable_reads for item in stats_list)
    if unparseable_rows:
        print(f"- 无法解析 read_count 的无效行: {format_int(unparseable_rows)}")

    print()
    print("[无效 NT 原因汇总]")
    nonzero_invalid = [row for row in invalid_rows if int(row["invalid_nt_rows"]) > 0]
    if not nonzero_invalid:
        print("- 未发现无效 NT 序列。")
    else:
        for row in nonzero_invalid:
            print(
                f"- {row['translate_status']}（{row['description_zh']}）: "
                f"NT行 {format_int(int(row['invalid_nt_rows']))} "
                f"({format_pct(float(row['invalid_nt_row_ratio_among_invalid']))} of invalid), "
                f"reads {format_int(int(row['invalid_read_count']))} "
                f"({format_pct(float(row['invalid_read_ratio_among_all_reads']), 4)} of all reads)"
            )
    ...
    top_n = args.top_n_low_quality
    if top_n > 0:
        print()
        print(f"[最低 valid_read_ratio 样本 Top {top_n}]")
        for item in sorted(stats_list, key=lambda value: value.valid_read_ratio)[:top_n]:
            print(
        ...
        print()
        print(f"[最低 valid_nt_row_ratio 样本 Top {top_n}]")
        for item in sorted(stats_list, key=lambda value: value.valid_nt_row_ratio)[:top_n]:
            print(
                f"- {item.sample_id}: valid_nt_row_ratio={item.valid_nt_row_ratio:.4f}, "
                f"valid_read_ratio={item.valid_read_ratio:.4f}, "
                f"invalid rows={format_int(item.invalid_nt_rows)}"
            )
    ...
    warn_list = [
        item for item in stats_list if item.qc_pass_01 and item.has_warning_01
    ]
    print()
    print("[其他 warning 样本，建议人工关注]")
    if not warn_list:
        print("- 无。")
    else:
        for item in warn_list[:10]:
            print(f"- {item.sample_id}: {item.qc_warning_01}")
        if len(warn_list) > 10:
            print(f"- 其余 {len(warn_list) - 10} 个见 01_sample_qc_status.csv")
    ...
    if not abnormal_norm:
        print(
            f"- PASS：全部样本 frequency_norm 与 read_fraction 求和均在 "
            f"{args.norm_tolerance:g} 容差内接近 1。"
        )
    else:
    ...
    print()
    print("[结果解释提示]")
    overall_valid_nt = safe_ratio(total_valid_rows, total_input_rows)
    overall_valid_reads = safe_ratio(total_valid_reads, total_reads)
    if overall_valid_reads >= args.min_valid_read_ratio:
        print(
            "- 总体有效 reads 比例达到设定阈值。即使无效 NT 行很多，"
            "若其 reads 占比很低，通常说明无效记录主要是低丰度 clone。"
        )
    else:
        print(
            "- WARNING：总体有效 reads 比例低于阈值。请在正式进入后续特征提取前，"
            "检查阅读框、CDR3 NT 定义和高丰度无效序列。"
        )
    if overall_valid_nt < args.min_valid_nt_row_ratio:
        print(
            "- WARNING：总体有效 NT 行比例低于阈值。clone 数量层面的无效记录较多，"
            "应结合无效原因和 reads 占比判断。"
        )
    print(
        "- length_not_multiple_of_3 与 stop_codon 通常对应非生产性或无法直接翻译的 CDR3，"
        "不应为了保留数量而强行补碱基或改变阅读框。"
    )
    ...
    if args.dry_run:
        ...
        print("- 确认上述 QC 后，去掉 --dry-run 即可正式生成结果。")
    else:
        print("[输出文件]")
        ...
        print(f"- 无效原因样本明细: {reports['invalid_by_sample']}")
        print(f"- 归一化检查: {reports['norm_check']}")
        print(f"- 建议排除样本: {reports['exclude_samples']}")
        if args.invalid_example_per_reason > 0:
            print(f"- 无效序列示例: {reports['invalid_examples']}")
...
def write_markdown_summary(
    ...
    lines: List[str] = [
        "# 01_AA_clone_table Build Summary",
        "",
        "## Run configuration",
        "",
        ...
        f"- Invalid-row policy: `{args.invalid_policy}`",
        f"- Minimum valid-read warning threshold: `{args.min_valid_read_ratio}`",
        f"- Minimum valid-NT-row warning threshold: `{args.min_valid_nt_row_ratio}`",
        "",
        "## Overall QC",
        "",
...
def write_formal_reports(
    ...
    write_csv_atomic(reports["invalid_summary"], invalid_fieldnames, invalid_rows)

    by_sample_rows = invalid_by_sample_rows(all_sample_reason_stats)
    write_csv_atomic(
...
def main() -> int:
    ...
    if metadata_path is not None:
        ...
        if extras:
            print(
                f"WARNING: 输入目录另有 {len(extras)} 个不在 metadata 中的文件，"
                "本次不会处理。"
            )
    ...
    output_files = [
        output_dir / f"{sample_id}{args.output_suffix}" for sample_id in sample_ids
    ]
    ...
    global_reason_stats = {
        reason: InvalidReasonStats() for reason in INVALID_REASONS
    }
    all_sample_reason_stats: Dict[str, Dict[str, InvalidReasonStats]] = {}
    ...
    if args.write_invalid_log and not args.dry_run:
        invalid_writer_context = InvalidDetailWriter(
            invalid_log_path, max_rows=args.invalid_log_max_rows
        )
