...
"""
Batch-run all repeated nested-CV outer tasks for the RA / RA-ILD TRB project.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Batch-run all LOO outer-fold modeling tasks.",
...
def validate_completed_task(
    ...
    required_cols = {"model", "fit_converged", "roc_auc", "pr_auc"}
    ...
    if models != EXPECTED_MODELS:
        return False, f"unexpected model set: {sorted(models)}"

    if len(metrics) != 4:
        return False, f"expected 4 metric rows, observed {len(metrics)}"
    ...
    return True, "all expected files and checks passed"
...
def write_summary(
    ...
    lines.extend([
        ...
        "## Resume behavior",
        "",
        "- Re-running the same command skips tasks whose expected outputs are complete and whose four final models converged.",
...
def main() -> int:
    ...
    print("=" * 80, flush=True)
    print("05 LOO Batch Outer-Trial Runner", flush=True)
    print("=" * 80, flush=True)
