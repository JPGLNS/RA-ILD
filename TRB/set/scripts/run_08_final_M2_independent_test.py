...
"""
One-time independent validation of the locked final M2 model.
...
- The model is loaded from the step-07 joblib bundle and is never refitted.
...
LOCKED_ALPHA = 0.5
LOCKED_LAMBDA = 30.0
LOCKED_THRESHOLD = 0.509337


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Apply the locked final M2 model to the 51-sample independent test set once.",
    ...
    parser.add_argument("--expected-test-samples", type=int, default=51)
...
def load_and_validate_locked_model(
    ...
    masks = np.load(reference_masks_path)
    required_masks = {
...
def make_evaluation_figure(
    ...
    fig.suptitle("Independent Test Validation of the Locked Final M2 Model", fontsize=15)
...
def write_summary(
    ...
    lines = [
        "# 08 Independent Test Validation of Final M2",
        "",
        "## Locked analysis",
        "",
        f"- Model: `{MODEL_NAME}`",
...
def main() -> int:
    ...
    print("=" * 80, flush=True)
    print("08 Locked Final M2 — Independent Test Validation", flush=True)
    print("=" * 80, flush=True)
