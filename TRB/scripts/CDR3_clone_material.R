...
# 此脚本用来统计不同的material组的CDR3 clone数分布，以及梯度分析
...
files <- list.files(
  result_dir,
  pattern = "_TRB_CDR3_NT_frequency_error_correct\\.csv$"
)
...
clone_counts <- data.frame(
  ...
  clone_count = sapply(file.path(result_dir, files), function(f) {
    length(readLines(f))
  }),
...
clone_counts <- merge(
  clone_counts,
  metadata[, c("libraryid", "material", "cohort")],
  by = "libraryid"
)

# 创建分组变量
clone_counts$group_two <- clone_counts$material
clone_counts$group_four <- paste(
  clone_counts$material,
  clone_counts$cohort,
  sep = "-"
)
...
stats_two$label <- paste0(
  round(stats_two$mean, 0),
  " (",
  round(stats_two$min, 0),
  "-",
  round(stats_two$max, 0),
  ")"
)

# 计算四组summary stats
stats_four <- aggregate(
  clone_count ~ group_four,
  data = clone_counts,
  FUN = function(x) c(mean = mean(x), min = min(x), max = max(x))
)
...
stats_four$label <- paste0(
  round(stats_four$mean, 0),
  "\n(",
  round(stats_four$min, 0),
  "-",
  round(stats_four$max, 0),
  ")"
)

stats_two$y_pos <- stats_two$max + diff(range(clone_counts$clone_count)) * 0.08
stats_four$y_pos <- stats_four$max +
  diff(range(clone_counts$clone_count)) * 0.08
...
# ========== 图1：两组（PBMC vs buffercoat）==========
p1 <- ggplot(
  clone_counts,
  aes(x = group_two, y = clone_count, fill = group_two)
) +
  geom_boxplot(outlier.shape = NA, alpha = 0.6, width = 0.5) +
  geom_jitter(width = 0.1, size = 1.2, alpha = 0.5) +
  geom_text(
    data = stats_two,
    aes(x = group_two, y = y_pos, label = label),
    inherit.aes = FALSE,
    size = 3.5,
    fontface = "bold",
    color = "black"
  ) +
  scale_fill_manual(values = c("PBMC" = "#E64B35", "buffercoat" = "#4DBBD5")) +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.18))) +
  labs(x = "Material", y = "Number of CDR3 Clones") +
  ggtitle("CDR3 Clone Count Distribution: PBMC vs Buffercoat") +
...
ggsave("./TRB/PBMC+buffercoat.png", p1, width = 6, height = 5, dpi = 300)

# ========== 图2：四组（PBMC-RA, PBMC-ILD, buffercoat-RA, buffercoat-ILD）==========
p2 <- ggplot(
  clone_counts,
  aes(x = group_four, y = clone_count, fill = group_four)
) +
  geom_boxplot(outlier.shape = NA, alpha = 0.6, width = 0.5) +
  geom_jitter(width = 0.1, size = 1.2, alpha = 0.5) +
  geom_text(
    data = stats_four,
    aes(x = group_four, y = y_pos, label = label),
    inherit.aes = FALSE,
    size = 3.5,
    fontface = "bold",
    color = "black"
  ...
  scale_fill_manual(
    values = c(
      "PBMC-RA" = "#E64B35",
      "PBMC-ILD" = "#F39B7F",
      "buffercoat-RA" = "#4DBBD5",
      "buffercoat-ILD" = "#7EC8E3"
    )
  ) +
  scale_y_continuous(expand = expansion(mult = c(0.05, 0.18))) +
...
ggsave("./TRB/PBMC+buffercoat+cohort.png", p2, width = 7, height = 5, dpi = 300)

cat("\n图片已保存至:\n")
cat("  ./TRB/PBMC+buffercoat.png\n")
cat("  ./TRB/PBMC+buffercoat+cohort.png\n")
...
names(all_freqs) <- gsub(
  "_TRB_CDR3_NT_frequency_error_correct\\.csv$",
  "",
  files
)
...
diff_data <- data.frame(
  top_n = integer(),
  PBMC_mean = numeric(),
  buffercoat_mean = numeric(),
  PBMC_RA_mean = numeric(),
  PBMC_ILD_mean = numeric(),
  buffercoat_RA_mean = numeric(),
  buffercoat_ILD_mean = numeric(),
  stringsAsFactors = FALSE
)
...
for (top_n in gradient_values) {
  ...
  diff_data <- rbind(
    ...
    data.frame(
      ...
      buffercoat_mean = s2$mean[s2$group_two == "buffercoat"],
      PBMC_RA_mean = s4$mean[s4$group_four == "PBMC-RA"],
      PBMC_ILD_mean = s4$mean[s4$group_four == "PBMC-ILD"],
      buffercoat_RA_mean = s4$mean[s4$group_four == "buffercoat-RA"],
      buffercoat_ILD_mean = s4$mean[s4$group_four == "buffercoat-ILD"],
      stringsAsFactors = FALSE
    )
  )
  ...
  p_top2 <- ggplot(
    top_data,
    aes(x = group_two, y = cum_frequency, fill = group_two)
  ) +
    geom_boxplot(outlier.shape = NA, alpha = 0.6, width = 0.5) +
    geom_jitter(width = 0.1, size = 1.2, alpha = 0.5) +
    geom_text(
    ...
    scale_fill_manual(
      values = c("PBMC" = "#E64B35", "buffercoat" = "#4DBBD5")
    ) +
    scale_y_continuous(expand = expansion(mult = c(0.05, 0.18))) +
    labs(
      x = "Material",
      y = paste0("Cumulative Frequency (Top ", top_n, " Clones)")
    ) +
    ggtitle(paste0(
      "Top ",
      top_n,
      " CDR3 Clone Cumulative Frequency: PBMC vs Buffercoat"
    )) +
    theme_classic(base_size = 14) +
    theme(
      legend.position = "none",
  ...
  fname2 <- paste0(gradient_dir, "PBMC+buffercoat_top", top_n, ".png")
  ggsave(fname2, p_top2, width = 6, height = 5, dpi = 300)

  # --- 四组箱线图 ---
  p_top4 <- ggplot(
    top_data,
    aes(x = group_four, y = cum_frequency, fill = group_four)
  ) +
    geom_boxplot(outlier.shape = NA, alpha = 0.6, width = 0.5) +
    geom_jitter(width = 0.1, size = 1.2, alpha = 0.5) +
    geom_text(
    ...
    ggtitle(paste0(
      "Top ",
      top_n,
      " CDR3 Clone Cumulative Frequency by Material and Cohort"
    )) +
    theme_classic(base_size = 14) +
    theme(
      legend.position = "none",
  ...
  fname4 <- paste0(gradient_dir, "PBMC+buffercoat+cohort_top", top_n, ".png")
...
# ====================================================================
# 第三部分：差值曲线图 — PBMC均值 - buffercoat均值 随梯度变化
# ====================================================================
cat("\n\n########## 差值曲线图 ##########\n")

diff_data$diff_two <- diff_data$PBMC_mean - diff_data$buffercoat_mean
diff_data$diff_RA <- diff_data$PBMC_RA_mean - diff_data$buffercoat_RA_mean
diff_data$diff_ILD <- diff_data$PBMC_ILD_mean - diff_data$buffercoat_ILD_mean
...
p_diff1 <- ggplot(diff_data, aes(x = top_n, y = diff_two)) +
  ...
  ggtitle("PBMC − Buffercoat: Mean Cumulative Frequency Difference") +
  theme_classic(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 10),
    axis.text = element_text(color = "black"),
...
ggsave(
  paste0(gradient_dir, "diff_curve_PBMC_vs_buffercoat.png"),
  p_diff1,
  width = 8,
  height = 5,
  dpi = 300
...
diff_long$label_vjust <- stagger_vjust(diff_data$diff_RA) # 每个梯度两个点用相同偏移方向
...
p_diff2 <- ggplot(
  diff_long,
  aes(x = top_n, y = diff_value, color = cohort_type, linetype = cohort_type)
) +
  geom_line(linewidth = 1) +
  geom_point(size = 2.5) +
  geom_text(
    aes(
      label = sprintf("%.2f", diff_value),
      color = cohort_type,
      vjust = label_vjust
    ),
    size = 3.2,
  ...
  labs(
    x = "Top N Clones",
    y = "Mean Cumulative Frequency Difference",
    color = "Cohort",
    linetype = "Cohort"
  ) +
  ggtitle("PBMC − Buffercoat: Mean Cumulative Frequency Difference by Cohort") +
  theme_classic(base_size = 14) +
  theme(
    plot.title = element_text(hjust = 0.5, face = "bold", size = 10),
    axis.text = element_text(color = "black"),
...
ggsave(
  paste0(gradient_dir, "diff_curve_by_cohort.png"),
  p_diff2,
  width = 8,
  height = 5,
  dpi = 300
