...
The script:
  1. reads one receptor's fixed training AA-clone tables and training metadata;
...
SCRIPT_VERSION = "1.2.1-RECEPTOR"
AA_INPUT_SUFFIX = "_IGH-without-DJ_CDR3_AA_clone_table.csv"
AA_SET = set("ACDEFGHIKLMNPQRSTVWY")

DEFAULT_ROOT = Path("/data/users/chenhaisheng/RA-ILD/IGH")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate one receptor public CDR3-AA threshold stability on training data.",
    ...
    parser.add_argument(
        ...
        choices=["TRB", "IGH"],
        required=True,
        help="Receptor label written to manifests and reports.",
    )
...
def build_manifest_signature(
    ...
    return {
        ...
        "receptor": receptor,
        "aa_input_suffix": aa_input_suffix,
...
def build_or_load_cache(
    ...
    if not args.keep_temp:
        # The cache files and manifest have already been written at this point.
        # On some filesystems or cleanup setups, the temporary directory may
        # already have disappeared. Treat that as a successful cleanup rather
        # than failing the completed cache build.
        if temp_root.exists():
            shutil.rmtree(temp_root)
            print(f"[缓存] 已清理临时排序目录: {temp_root}")
        else:
            print(f"[缓存] 临时排序目录已不存在，跳过清理: {temp_root}")
    else:
...
def build_markdown_summary(
    ...
    lines: List[str] = [
        f"# 03 {args.receptor} Public CDR3-AA Threshold Stability Summary",
        "",
        "## Scope",
        "",
...
def main() -> int:
    ...
    print("")
    print("=" * 80)
    print(f"03A {args.receptor} Public CDR3-AA 阈值稳定性评估")
    print("=" * 80)
    ...
    print(f"注意：脚本只读取固定{args.receptor}训练集，不读取test目录，也不计算模型AUC。")
    print("")
