...
# -*- coding: utf-8 -*-
"""
Select and finalize a fixed 70/30 train/test split by repeated stratified sampling.

Designed for paired RA / RA-ILD TRB+IGH repertoire metadata. The default stratification
variables are cohort + batch. Batch is used only to balance the split; this
script does not perform feature extraction or model training. It evaluates all
requested seeds, selects the split with the lowest metadata-imbalance score, and
writes that selected split as the final fixed train/test metadata.
...
Example:
    python3 train_test_stratified_split_TRB_IGH.py \
        --metadata /data/users/chenhaisheng/RA-ILD/TRB_IGH/metadata.csv \
        --output-dir /data/users/chenhaisheng/RA-ILD/TRB_IGH/set \
        --train-ratio 0.70 \
        --seed-start 1 \
        --seed-end 5000 \
...
# Penalties added when the test set completely misses a non-singleton category
# that exists in the full metadata. A category represented by only one sample is
# structurally unable to appear in both sets and is reported without a penalty.
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="auto",
        help=(
            "Unique split-unit ID column. With 'auto', patient is preferred, "
            "followed by libraryid and sample_id."
        ),
    )
    parser.add_argument(
        "--missing-policy",
...
def resolve_id_column(metadata: pd.DataFrame, requested: str) -> str:
    ...
    for candidate in ("patient", "libraryid", "sample_id"):
        if candidate in metadata.columns:
            return candidate
    raise SplitError(
        "--id-col auto could not find 'patient', 'libraryid', or 'sample_id'. "
...
def read_and_validate_metadata(
    ...
    if id_col != "patient":
        raise SplitError(
            "Paired TRB+IGH splitting must use patient as the split unit. "
            "Run with --id-col patient."
        )
    ...
    # A paired row must map one patient to exactly one TRB library and one IGH library.
    for paired_id_col in ("trb_libraryid", "igh_libraryid"):
        paired_missing = missing_mask(metadata[paired_id_col])
        if paired_missing.any():
            example_rows = metadata.index[paired_missing].tolist()[:10]
            raise SplitError(
                f"Column '{paired_id_col}' contains {int(paired_missing.sum())} "
                f"missing/blank value(s), for example input row indices {example_rows}."
            )
        paired_ids = normalized_text(metadata[paired_id_col])
        paired_duplicate_mask = paired_ids.duplicated(keep=False)
        if paired_duplicate_mask.any():
            duplicates = sorted(paired_ids[paired_duplicate_mask].unique().tolist())
            raise SplitError(
                f"Column '{paired_id_col}' is not unique. Duplicate ID example(s): "
                + ", ".join(duplicates[:20])
            )

    columns_checked_for_missing = sorted(
        set(strata_cols) | {"cohort", "batch", "igh_batch", "split_batch", "material", "sex", "age"}
    )
    missing_counts = {
    ...
    batch_values = normalized_text(metadata["batch"])
    split_batch_values = normalized_text(metadata["split_batch"])
    split_mismatch = batch_values.ne(split_batch_values)
    if split_mismatch.any():
        examples = metadata.loc[
            split_mismatch, ["patient", "batch", "split_batch"]
        ].head(10)
        if "batch" in strata_cols and "split_batch" not in strata_cols:
            raise SplitError(
                "The requested stratification uses 'batch', but batch and split_batch "
                "differ for one or more rows. Resolve the discrepancy before splitting. "
                "Examples:\n"
                + examples.to_string(index=False)
            )
        warnings.append(
            f"batch and split_batch differ for {int(split_mismatch.sum())} row(s); "
            f"the requested stratification columns are {', '.join(strata_cols)}."
        )

    igh_batch_values = normalized_text(metadata["igh_batch"])
    discordant = batch_values.ne(igh_batch_values)
    if discordant.any():
        patients = metadata.loc[discordant, "patient"].astype(str).tolist()
        warnings.append(
            f"TRB/main batch and IGH batch differ for {int(discordant.sum())} "
            f"paired patient(s): {', '.join(patients[:20])}. Stratification uses 'batch'."
        )
...
def score_split(
    ...
    for spec in specs:
        ...
        if spec.name in TEST_MISSING_CATEGORY_PENALTIES:
            # Penalize only categories with at least two samples. Singleton
            # categories cannot be represented in both train and test. Such
            # structurally unavoidable absences are reported without penalty.
            missing_non_singleton = int(
                np.sum((spec.overall_counts >= 2) & (test_counts == 0))
            )
            missing_singleton = int(
                np.sum((spec.overall_counts == 1) & (test_counts == 0))
            )
            metrics[f"missing_test_categories_{spec.name}"] = float(
                missing_non_singleton
            )
            metrics[f"missing_test_singleton_categories_{spec.name}"] = float(
                missing_singleton
            )
            missing_test_category_penalty += (
                missing_non_singleton * TEST_MISSING_CATEGORY_PENALTIES[spec.name]
            )
...
def build_summary(
    ...
    if patient_check and int(patient_check["overlap_count"]) > 0:
        warnings.append(
            ...
            f"{patient_check['overlap_count']}. The split unit must be patient."
        )

    lines: List[str] = []
    lines.append("# Paired TRB+IGH Train/Test Split Summary")
    lines.append("")
    ...
    lines.append("")
    lines.append("## Selected and finalized split")
    lines.append("")
    lines.append(f"- Input paired patients: **{len(metadata)}**")
    ...
    lines.append("- Selection used metadata balance only; no model-performance metric was used.")
    lines.append("- The train/test CSV files below are the finalized fixed split for downstream analysis.")
    if tied > 1:
    ...
    lines.append("")
    lines.append("Test missing-category penalties per absent non-singleton category (overall n >= 2):")
    lines.append("")
    penalty_rows = [
    ...
    check_rows = [
        ...
        {
            "check": "train/test patient-ID union covers all input patients",
    ...
    if patient_check is not None:
        lines.append("")
        lines.append("### Patient-level split integrity")
        lines.append("")
        lines.append(
        ...
        lines.append(
            "- The split unit for the paired analysis should be `patient`; TRB and IGH library IDs remain linked within each row."
        )
    ...
    table_sections = [
        ...
        ("Batch distribution used for splitting", ("batch",)),
        ("IGH batch distribution (QC only)", ("igh_batch",)),
...
def main() -> int:
    ...
    try:
        ...
        print(summary)
        print("\nCompleted successfully. The selected paired TRB+IGH train/test split has been finalized and written to disk.")
        return 0
