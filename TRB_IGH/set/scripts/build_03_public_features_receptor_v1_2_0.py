...
"""
Build final step-03 public CDR3-AA files for one receptor (TRB or IGH).

TRAIN mode uses only the fixed training sparse cache and the confirmed
threshold scheme. TEST mode applies the fixed training reference sets to
independent test samples without using test labels to construct references.

This engine is intended to be called by build_03_public_features_TRB_IGH.py.
"""
...
SCRIPT_VERSION = "1.2.0-RECEPTOR"
AA_INPUT_SUFFIXES = {
    "TRB": "_TRB_CDR3_AA_clone_table.csv",
    "IGH": "_IGH-without-DJ_CDR3_AA_clone_table.csv",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Build final step-03 public CDR3-AA files for TRB or IGH.",
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument("--mode", required=True, choices=["train", "test"])
    parser.add_argument("--receptor", required=True, choices=["TRB", "IGH"])
    ...
    parser.add_argument("--aa-input-suffix", default=None)
    parser.add_argument("--catalog-chunk-size", type=int, default=100_000)
    parser.add_argument("--overwrite", action="store_true")

    parser.add_argument("--metadata", required=True)
    parser.add_argument("--summary-file", required=True)
    parser.add_argument("--aa-dir", required=True)
    parser.add_argument("--output-dir", required=True)
    parser.add_argument("--stability-configuration", default=None)
    parser.add_argument("--stability-cache-dir", default=None)
    parser.add_argument("--reference-file", default=None)
    parser.add_argument("--reference-definition", default=None)

    args = parser.parse_args()
    args.receptor = args.receptor.upper()
    args.aa_input_suffix = args.aa_input_suffix or AA_INPUT_SUFFIXES[args.receptor]
    ...
    if args.mode == "train":
        if not args.stability_configuration:
            parser.error("--stability-configuration is required in train mode.")
        if not args.stability_cache_dir:
            parser.error("--stability-cache-dir is required in train mode.")
    else:
        if not args.reference_file:
            parser.error("--reference-file is required in test mode.")
        if not args.reference_definition:
            parser.error("--reference-definition is required in test mode.")
    return args
...
def validate_stability_inputs(
    ...
    """Validate that stability configuration and cache belong to this receptor run."""
...
def write_train_summary(
    ...
    lines = [
        f"# 03 {EXPECTED_RECEPTOR} Public CDR3-AA Build Summary — TRAIN",
        "",
        "## Scope",
        "",
...
def write_test_summary(
    ...
    lines = [
        f"# 03 {EXPECTED_RECEPTOR} Public CDR3-AA Build Summary — TEST",
        "",
        "## Scope",
        "",
...
def run_train(args: argparse.Namespace) -> int:
    ...
    print("=" * 80)
    print(f"03 {EXPECTED_RECEPTOR} Public CDR3-AA 正式构建 — TRAIN")
    print("=" * 80)
...
def run_test(args: argparse.Namespace) -> int:
    ...
    print("=" * 80)
    print(f"03 {EXPECTED_RECEPTOR} Public CDR3-AA 正式构建 — TEST")
    print("=" * 80)
...
def main() -> int:
    global EXPECTED_RECEPTOR
    args = parse_args()
    EXPECTED_RECEPTOR = args.receptor
    if args.mode == "train":
