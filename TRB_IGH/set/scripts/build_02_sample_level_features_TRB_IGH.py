...
"""
Build paired TRB and IGH 02_sample_level_features from step-01 outputs.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "构建配对队列的 TRB 与 IGH 02_sample_level_features。"
            "两种组库分别建立核心特征和3-mer矩阵；TRAIN分别建立词表，"
            "TEST分别复用对应的TRAIN词表。"
        ),
        formatter_class=argparse.ArgumentDefaultsHelpFormatter,
    )
    parser.add_argument('--mode', choices=['train','test'], required=True,
                        help='TRAIN建立各受体词表；TEST复用各自TRAIN词表。')
    parser.add_argument('--receptors', choices=['both','trb','igh'], default='both',
                        help='处理两种组库或只处理其中一种。')
    parser.add_argument('--metadata', required=True, help='配对训练集或测试集metadata。')
    parser.add_argument('--patient-col', default='patient', help='患者ID列，仅用于配对完整性检查。')
    ...
    parser.add_argument('--trb-aa-dir', default=None, help='TRB 01_AA_clone_table目录。')
    parser.add_argument('--igh-aa-dir', default=None, help='IGH 01_AA_clone_table目录。')
    parser.add_argument('--trb-summary-file', default=None, help='TRB 01_AA_clone_table_summary.csv。')
    parser.add_argument('--igh-summary-file', default=None, help='IGH 01_AA_clone_table_summary.csv。')
    parser.add_argument('--trb-output-dir', default=None, help='TRB 02输出目录。')
    parser.add_argument('--igh-output-dir', default=None, help='IGH 02输出目录。')
    parser.add_argument('--trb-aa-input-suffix', default='_TRB_CDR3_AA_clone_table.csv')
    parser.add_argument('--igh-aa-input-suffix', default='_IGH-without-DJ_CDR3_AA_clone_table.csv')
    parser.add_argument('--trb-vocabulary-file', default=None,
                        help='TEST模式使用的TRB训练词表。')
    parser.add_argument('--igh-vocabulary-file', default=None,
                        help='TEST模式使用的IGH训练词表。')
    ...
    parser.add_argument('--max-kmers', type=int, default=500,
                        help='每种受体分别最多保留的3-mer数量；0表示不限制。')
    parser.add_argument('--norm-tolerance', type=float, default=1e-8)
    parser.add_argument('--summary-mismatch-policy', choices=['error','warn'], default='error')
    parser.add_argument('--invalid-aa-policy', choices=['error','skip'], default='error')
    parser.add_argument('--write-merged', action='store_true')
    parser.add_argument('--paired-summary-file', default=None,
                        help='联合运行摘要路径；默认写入两个输出目录的共同父目录。')
    ...
    active = ['TRB','IGH'] if args.receptors == 'both' else [args.receptors.upper()]
    for receptor in active:
        prefix=receptor.lower()
        for field in ('aa_dir','summary_file','output_dir'):
            if not getattr(args, f'{prefix}_{field}'):
                parser.error(f'--{prefix}-{field.replace("_", "-")} is required when {receptor} is active.')
        if args.mode == 'test' and not getattr(args, f'{prefix}_vocabulary_file'):
            parser.error(f'--{prefix}-vocabulary-file is required in TEST mode.')
    return args


def read_and_validate_paired_metadata(path: Path, patient_col: str,
                                      active_specs: Sequence[Tuple[str,str]]) -> pd.DataFrame:
    if not path.is_file():
    ...
    required={patient_col} | {id_col for _,id_col in active_specs}
    missing=sorted(required-set(df.columns))
    if missing:
        raise ValueError(f'配对metadata缺少必要列: {missing}')
    for col in required:
        values=df[col].astype('string').str.strip()
        if values.isna().any() or values.eq('').any():
            raise ValueError(f"metadata列'{col}'存在缺失或空值。")
        if values.duplicated().any():
            examples=values[values.duplicated(keep=False)].unique().tolist()[:10]
            raise ValueError(f"metadata列'{col}'存在重复，例如: {examples}")
        df[col]=values.astype(str)
    return df


def make_single_args(paired: argparse.Namespace, spec: ReceptorSpec) -> argparse.Namespace:
    return argparse.Namespace(
        mode=paired.mode,
        aa_dir=str(spec.aa_dir),
        summary_file=str(spec.summary_file),
        metadata=str(Path(paired.metadata).expanduser().resolve()),
        id_col=spec.id_col,
        output_dir=str(spec.output_dir),
        aa_input_suffix=spec.aa_input_suffix,
        vocabulary_file=str(spec.vocabulary_file) if spec.vocabulary_file else None,
        min_kmer_sample_count=paired.min_kmer_sample_count,
        min_kmer_prevalence=paired.min_kmer_prevalence,
        min_kmer_unweighted_variance=paired.min_kmer_unweighted_variance,
        min_kmer_weighted_variance=paired.min_kmer_weighted_variance,
        max_kmers=paired.max_kmers,
        norm_tolerance=paired.norm_tolerance,
        summary_mismatch_policy=paired.summary_mismatch_policy,
        invalid_aa_policy=paired.invalid_aa_policy,
        write_merged=paired.write_merged,
        dry_run=paired.dry_run,
        overwrite=paired.overwrite,
        receptor_name=spec.name,
    )
...
def preflight_receptor(args: argparse.Namespace) -> None:
    ...
    if not aa_dir.is_dir():
        raise NotADirectoryError(f'AA clone表目录不存在: {aa_dir}')
    ...
    missing_summary=[x for x in sample_ids if x not in summary_df.index]
    if missing_summary:
        raise ValueError(f'{args.receptor_name}: metadata样本在01 summary缺失: {missing_summary[:20]}')
    missing_files=[aa_dir/f'{sample_id}{args.aa_input_suffix}' for sample_id in sample_ids
                   if not (aa_dir/f'{sample_id}{args.aa_input_suffix}').is_file()]
    if missing_files:
        preview='\n'.join(f'  - {p}' for p in missing_files[:20])
        raise FileNotFoundError(f'{args.receptor_name}: 缺失{len(missing_files)}个AA clone表:\n{preview}')
    if args.mode=='test':
        vocab=Path(args.vocabulary_file).expanduser().resolve()
        if not vocab.is_file():
            raise FileNotFoundError(f'{args.receptor_name}: 训练vocabulary不存在: {vocab}')
        read_training_vocabulary(vocab)
    outputs=expected_output_paths(output_dir,args.mode,args.write_merged)
    if not args.dry_run:
        check_overwrite(outputs,args.overwrite)


def run_one_receptor(args: argparse.Namespace) -> ReceptorRunResult:
    ...
    if extra_summary_ids:
        print(f'WARNING: {args.receptor_name} 01 summary中有{len(extra_summary_ids)}个不属于当前metadata的样本，将忽略。')
    aa_files={sample_id: aa_dir/f'{sample_id}{args.aa_input_suffix}' for sample_id in sample_ids}
    output_dir.mkdir(parents=True,exist_ok=True)
    outputs=expected_output_paths(output_dir,args.mode,args.write_merged)

    training_vocab_df=None
    retained_kmers_from_train=None
    retained_set=None
    if args.mode=='test':
        vocab_path=Path(args.vocabulary_file).expanduser().resolve()
        training_vocab_df,retained_kmers_from_train=read_training_vocabulary(vocab_path)
        retained_set=set(retained_kmers_from_train)

    print('\n'+'#'*78)
    print(f'开始处理 {args.receptor_name} 02_sample_level_features')
    print('#'*78)
    ...
    for index,sample_id in enumerate(sample_ids,start=1):
        ...
        print(f'[{args.receptor_name} {index:>3}/{len(sample_ids)}] {sample_id} | '
              f'AA={log.aa_clone_number:,} | reads={log.total_reads_valid_aa:,} | '
              f'Shannon={log.Shannon:.4f} | top1={log.top1_frequency:.6f} | '
              f'3mer={log.nonzero_unweighted_3mers:,} | {status} | CALCULATED')

    core_df=pd.DataFrame(core_rows)
    if args.mode=='train':
        vocab_df=determine_vocabulary(sample_ids,unweighted_by_sample,weighted_by_sample,args)
        retained_kmers=vocab_df.loc[vocab_df['keep'],'kmer'].astype(str).tolist()
        if not retained_kmers:
            raise ValueError(
                f'{args.receptor_name}: 训练集筛选后没有保留任何3-mer；'
                '请检查输入数据或筛选阈值。'
            )
    else:
        assert training_vocab_df is not None and retained_kmers_from_train is not None
        vocab_df=training_vocab_df.copy()
        retained_kmers=retained_kmers_from_train

    unweighted_df=build_kmer_matrix(sample_ids,unweighted_by_sample,retained_kmers,prefix='unweighted')
    weighted_df=build_kmer_matrix(sample_ids,weighted_by_sample,retained_kmers,prefix='weighted')
    merged_df=None
    if args.write_merged:
        merged_df=(core_df.merge(unweighted_df,on='sample_id',how='left',validate='one_to_one')
                   .merge(weighted_df,on='sample_id',how='left',validate='one_to_one'))

    for name,df in [('core',core_df),('unweighted',unweighted_df),('weighted',weighted_df)]:
        if df['sample_id'].tolist()!=sample_ids:
            raise RuntimeError(f'{args.receptor_name} {name}矩阵样本顺序与metadata不一致。')
        if df['sample_id'].duplicated().any():
            raise RuntimeError(f'{args.receptor_name} {name}矩阵sample_id存在重复。')
        numeric=df.drop(columns=['sample_id']).to_numpy(dtype=float)
        if not np.isfinite(numeric).all():
            raise RuntimeError(f'{args.receptor_name} {name}矩阵存在NaN或无穷值。')
    ...
    if not args.dry_run:
        ...
        write_dataframe_atomic(pd.DataFrame([asdict(x) for x in logs]),outputs['build_log'])
        if args.write_merged and merged_df is not None:
            write_dataframe_atomic(merged_df,outputs['merged'])
        markdown=make_markdown_summary(args,sample_ids,core_df,vocab_df,retained_kmers,logs,outputs,total_seconds)
        temp_summary=outputs['summary'].with_suffix('.md.tmp')
        temp_summary.write_text(markdown+'\n',encoding='utf-8')
        temp_summary.replace(outputs['summary'])

    print_final_report(args,core_df,vocab_df,retained_kmers,logs,outputs,total_seconds)
    return ReceptorRunResult(
        receptor=args.receptor_name,
        sample_count=len(sample_ids),
        core_column_count=core_df.shape[1],
        candidate_kmer_count=len(vocab_df),
        retained_kmer_count=len(retained_kmers),
        output_dir=output_dir,
        vocabulary_file=outputs.get('vocabulary') if args.mode=='train' else Path(args.vocabulary_file),
        merged_file=outputs.get('merged'),
        total_seconds=total_seconds,
    )


def common_summary_path(specs: Sequence[ReceptorSpec], requested: Optional[str]) -> Path:
    if requested:
        return Path(requested).expanduser().resolve()
    common=Path(os.path.commonpath([str(spec.output_dir) for spec in specs]))
    if common in {spec.output_dir for spec in specs}:
        common=common.parent
    return common/'02_TRB_IGH_sample_level_features_summary.md'


def write_paired_summary(path: Path,args: argparse.Namespace,metadata_rows: int,
                         results: Sequence[ReceptorRunResult]) -> None:
    lines=[
        '# Paired TRB+IGH 02 Sample-level Features Summary','',
        '## Configuration','',
        f'- Script version: `{SCRIPT_VERSION}`',
        f'- Mode: `{args.mode}`',
        f'- Paired metadata: `{Path(args.metadata).resolve()}`',
        f'- Paired patients: **{metadata_rows}**',
        f'- Receptors: `{args.receptors}`',
        f'- Maximum retained 3-mers per receptor: **{args.max_kmers}**',
        f'- Write merged matrices: `{args.write_merged}`','',
        '## Receptor results','',
        '| receptor | samples | core_columns | candidate_3mers | retained_3mers | runtime_seconds | output_dir |',
        '| --- | ---: | ---: | ---: | ---: | ---: | --- |',
    ]
    for result in results:
        lines.append(f'| {result.receptor} | {result.sample_count} | {result.core_column_count} | '
                     f'{result.candidate_kmer_count} | {result.retained_kmer_count} | '
                     f'{result.total_seconds:.2f} | `{result.output_dir}` |')
    lines += ['', '## Leakage control','',
              '- TRB and IGH vocabularies are selected independently using TRAIN samples only.',
              '- TEST reuses the fixed vocabulary of the corresponding receptor.',
              '- TEST-only 3-mers are ignored and absent TRAIN vocabulary features are filled with zero.',
              '- This stage does not merge TRB and IGH features and does not add cohort or clinical variables.','']
...
def main() -> int:
    ...
    active_names=['TRB','IGH'] if args.receptors=='both' else [args.receptors.upper()]
    id_pairs=[]
    if 'TRB' in active_names:
        id_pairs.append(('TRB',args.trb_id_col))
    if 'IGH' in active_names:
        id_pairs.append(('IGH',args.igh_id_col))
    metadata=read_and_validate_paired_metadata(metadata_path,args.patient_col,id_pairs)

    specs=[]
    if 'TRB' in active_names:
        specs.append(ReceptorSpec('TRB',args.trb_id_col,
            Path(args.trb_aa_dir).expanduser().resolve(),
            Path(args.trb_summary_file).expanduser().resolve(),
            Path(args.trb_output_dir).expanduser().resolve(),
            args.trb_aa_input_suffix,
            Path(args.trb_vocabulary_file).expanduser().resolve() if args.trb_vocabulary_file else None))
    if 'IGH' in active_names:
        specs.append(ReceptorSpec('IGH',args.igh_id_col,
            Path(args.igh_aa_dir).expanduser().resolve(),
            Path(args.igh_summary_file).expanduser().resolve(),
            Path(args.igh_output_dir).expanduser().resolve(),
            args.igh_aa_input_suffix,
            Path(args.igh_vocabulary_file).expanduser().resolve() if args.igh_vocabulary_file else None))

    single_args=[make_single_args(args,spec) for spec in specs]
    summary_path=common_summary_path(specs,args.paired_summary_file)
    # Global preflight before either receptor writes output.
    if not args.dry_run and summary_path.exists() and not args.overwrite:
        raise FileExistsError(f'联合summary已存在，请使用--overwrite: {summary_path}')
    for item in single_args:
        preflight_receptor(item)

    results=[]
    for item in single_args:
        results.append(run_one_receptor(item))

    if not args.dry_run:
        write_paired_summary(summary_path,args,len(metadata),results)

    print('\n'+'='*78)
    print('配对 TRB+IGH 02_sample_level_features：联合总览')
    print('='*78)
    print(f'运行模式: {args.mode.upper()}')
    print(f'metadata患者数: {len(metadata)}')
    for result in results:
        print(f'{result.receptor}: samples={result.sample_count}, core_columns={result.core_column_count}, '
              f'candidate_3mers={result.candidate_kmer_count}, retained_3mers={result.retained_kmer_count}')
    if args.dry_run:
        print('本次为dry-run：未写入02结果文件。')
    else:
        print(f'联合summary: {summary_path}')
