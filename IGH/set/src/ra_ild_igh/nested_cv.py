#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Leakage-controlled nested cross-validation orchestration for IGH V2.
...
def tune_models(
    ...
    if not candidates:
        raise NestedCVError(
            "At least one hyperparameter candidate is required."
        )
    ...
    for model_index, (model_name, folds) in enumerate(prepared.items()):
        if not folds:
            raise NestedCVError(
                f"No prepared inner folds for {model_name}."
            )
        for candidate in candidates:
            ...
            for inner_fold, fold_data in folds.items():
                ...
                probability = fit.predict_probability(
                    fold_data.X_valid
                )
                fold_aucs.append(
            ...
            pooled_y = prediction_frame[
                "true_label"
            ].to_numpy(int)
            pooled_probability = prediction_frame[
                "probability"
            ].to_numpy(float)
            ...
            tuning_rows.append(
                {
                    ...
                    "l1_ratio_alpha": float(candidate.alpha),
                    "lambda": float(candidate.lambda_value),
                    "C_inverse_lambda": float(
                        candidate.inverse_lambda_c
                    ),
