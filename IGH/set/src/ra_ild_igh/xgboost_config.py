#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Configuration and deterministic Random Search candidate-bank support for Batch 15."""
...
def find_repository_root(start: Path) -> Path:
    ...
    for candidate in (current, *current.parents):
        if (candidate / ".git").exists() or ((candidate / "IGH").is_dir() and (candidate / "IGH").exists()):
            return candidate
...
def build_xgboost_candidate_bank(engine: Mapping[str, Any]) -> Tuple[List[XGBoostCandidate], int]:
    ...
    if strategy == "grid":
        ...
        if replacement:
            raise XGBoostConfigError("Batch 15 v1 requires random search replacement=false")
        if n_iter > len(pool):
...
def validate_xgboost_mapping(raw: Mapping[str, Any], *, require_candidate_bank: bool = False) -> None:
    ...
    if str(experiment.get("receptor")) != "IGH":
        raise XGBoostConfigError("experiment.receptor must be IGH")
    ...
    if objective != "binary:logistic":
        raise XGBoostConfigError("Batch 15 requires objective='binary:logistic'")
    if bool(engine.get("early_stopping", False)):
        raise XGBoostConfigError("Batch 15 v1 requires model_engine.early_stopping=false")
