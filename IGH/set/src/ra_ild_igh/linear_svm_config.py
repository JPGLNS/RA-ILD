#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Configuration loading and candidate expansion for IGH Linear SVM schemes."""
...
def validate_linear_svm_mapping(raw: Mapping[str, Any]) -> None:
    ...
    if _nonempty_string(experiment, "receptor", "experiment") != "IGH":
        raise LinearSVMConfigError("experiment.receptor must be IGH")
    for section in (
        "data",
        "public_reference",
        "cross_validation",
    ...
    if engine.get("fit_intercept", True) is not True:
        raise LinearSVMConfigError("Batch 14 requires model_engine.fit_intercept=true")
