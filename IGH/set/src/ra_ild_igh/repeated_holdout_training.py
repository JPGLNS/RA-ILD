...
* combines the old 123-sample train matrix and 51-sample test matrix into a
  static-feature-only 169-sample matrix;
* builds a deterministic full-cohort sparse sequence representation;
* converts the three frozen 120/49 assignments into V2-compatible outer and
...
current training partition by :mod:`ra_ild_igh.nested_cv`.
"""
...
from .specifications import select_static_igh_features
...
def load_training_bundle_spec(path: Path, *, repository_root: Path) -> TrainingBundleSpec:
    ...
    allowed = resolve_project_path("IGH/set/training_bundles", root)
    try:
    ...
    scheme.update(
        {
            "id": spec.scheme_id,
            "description": (
                "Three frozen 120/49 repeated holdouts with automatic split-specific "
