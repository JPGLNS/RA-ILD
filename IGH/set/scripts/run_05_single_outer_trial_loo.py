...
"""
Run one complete outer-fold trial for the RA / RA-ILD IGH project.
...
Models:
    M0_clinical:
        age + sex
    M1_static_igh:
        age + sex + static step-02 IGH features
...
MODEL_LABELS = {
    "M0_clinical": "age + sex",
    "M1_static_igh": "age + sex + static IGH",
    "M2_static_igh_public": "age + sex + static IGH + dynamic public",
    "M3_static_igh_public_material": (
        "age + sex + material + static IGH + dynamic public"
    ),
}
...
def load_and_validate_reference_definition(args: argparse.Namespace) -> Tuple[Path, Dict[str, object]]:
    """Validate that the frozen step-03 definition matches this IGH model run."""
    path = Path(args.reference_definition).expanduser().resolve()
    ensure_file(path, "reference definition")
    definition = json.loads(path.read_text(encoding="utf-8"))

    receptor = str(definition.get("receptor", "")).upper()
    if receptor != EXPECTED_RECEPTOR:
        raise ValueError(
            f"Reference definition receptor={receptor or '<missing>'}; "
            f"expected {EXPECTED_RECEPTOR}."
        )

    expected_suffix = args.clone_file_pattern.replace("{sample_id}", "")
    observed_suffix = str(definition.get("aa_input_suffix", ""))
    if observed_suffix != expected_suffix:
        raise ValueError(
            "Reference definition AA suffix does not match --clone-file-pattern: "
    ...
    if str(scheme.get("name", "")) != "main":
        raise ValueError(
            f"Reference definition scheme={scheme.get('name')}; expected main."
        )
    expected_scheme = {
        ...
        "specific_prevalence_delta": float(args.specific_prevalence_delta),
    }
    for key, expected in expected_scheme.items():
        if key not in scheme:
            raise ValueError(f"Reference definition scheme missing {key}.")
        observed = float(scheme[key])
        if not math.isclose(observed, float(expected), rel_tol=0.0, abs_tol=1e-12):
            raise ValueError(
                f"Reference definition {key}={observed}; command expects {expected}."
            )

    n_training = int(definition.get("n_training_samples", -1))
    if args.expected_samples > 0 and n_training != args.expected_samples:
        raise ValueError(
            f"Reference definition training samples={n_training}; "
            f"expected {args.expected_samples}."
        )

    candidate_count = int(definition.get("candidate_sequence_count", -1))
    if args.expected_catalog_size > 0 and candidate_count != args.expected_catalog_size:
        raise ValueError(
            f"Reference definition candidate count={candidate_count}; "
            f"expected {args.expected_catalog_size}."
        )

    sizes = definition.get("reference_set_sizes", {})
    checks = {
        "global_public": args.expected_global_size,
        "RA_specific_ref": args.expected_ra_specific_size,
        "ILD_specific_ref": args.expected_ild_specific_size,
        "between_group_shared": args.expected_shared_size,
    }
    for key, expected in checks.items():
        if expected > 0 and int(sizes.get(key, -1)) != expected:
            raise ValueError(
                f"Reference definition {key}={sizes.get(key)}; expected {expected}."
            )
...
def static_igh_features_from_manifest(
    ...
    if "receptor" not in manifest.columns:
        raise ValueError("Feature manifest missing receptor column.")
    receptors = set(manifest["receptor"].astype(str).str.upper())
    if receptors != {EXPECTED_RECEPTOR}:
        raise ValueError(
            f"Feature manifest receptor values={sorted(receptors)}; "
            f"expected only {EXPECTED_RECEPTOR}."
        )
    ...
    if not selected:
        raise ValueError("No static IGH features were selected from the manifest.")
    if len(selected) != len(set(selected)):
        raise ValueError("Static IGH feature list contains duplicates.")
    return selected
...
def cache_matches(
    ...
    try:
        ...
        return False
    return (
        meta.get("receptor") == EXPECTED_RECEPTOR
        and meta.get("sample_ids") == list(sample_ids)
        and meta.get("catalog_sha256") == sha256sum(catalog_path)
        and meta.get("clone_file_pattern") == clone_file_pattern
        and meta.get("reference_definition_sha256") == reference_definition_sha256
...
def build_or_load_sparse_cache(
    ...
    valid_cache = (
        ...
        and cache_matches(
            ...
            catalog_path,
            clone_signatures,
            args.clone_file_pattern,
            sha256sum(Path(args.reference_definition).expanduser().resolve()),
        )
    )
    ...
    meta = {
        ...
        "reference_definition_sha256": sha256sum(
            Path(args.reference_definition).expanduser().resolve()
        ),
...
def output_paths(output_dir: Path) -> Dict[str, Path]:
    return {
        ...
        "static_feature_list": output_dir / "05_static_igh_feature_list.csv",
...
def write_summary(
    ...
    lines = [
        ...
        "## Feature definitions",
        "",
        f"- Static IGH features selected from manifest: **{n_static}**",
...
def main() -> int:
    ...
    reference_definition_path, reference_definition = (
        load_and_validate_reference_definition(args)
    )
    ...
    static_features = static_igh_features_from_manifest(manifest, base)
    ...
    print("=" * 80)
    print("05 IGH Single Outer-Fold Modeling Trial (LOO public)")
    print("=" * 80)
    ...
    print(f"Static IGH predictors: {len(static_features)}")
    print(
    ...
    config = {
        ...
        "reference_definition": str(reference_definition_path),
        "reference_definition_sha256": sha256sum(reference_definition_path),
        "reference_definition_validated": True,
        "reference_definition_snapshot": reference_definition,
