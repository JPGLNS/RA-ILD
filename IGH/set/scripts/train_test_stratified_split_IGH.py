...
Designed for RA / RA-ILD IGH repertoire metadata. The default stratification
...
Example:
    python3 train_test_stratified_split.py \
        --metadata /data/users/chenhaisheng/RA-ILD/IGH/metadata.csv \
        --output-dir /data/users/chenhaisheng/RA-ILD/IGH/set \
        --train-ratio 0.70 \
        --seed-start 1 \
        --seed-end 5000 \
...
def score_split(
    ...
    for spec in specs:
        ...
        if spec.name in TEST_MISSING_CATEGORY_PENALTIES:
            ...
            # categories cannot be represented in both train and test; for IGH,
            # this prevents the one-sample batch6 category from adding an
            # unavoidable penalty when --singleton-policy train is used.
...
def build_summary(
    ...
    lines: List[str] = []
    lines.append("# IGH Train/Test Split Summary")
    lines.append("")
...
def main() -> int:
    ...
    try:
        ...
        print(summary)
        print("\nCompleted successfully. The selected IGH train/test split has been finalized and written to disk.")
        return 0
