...
The script:
  1. reads the fixed IGH training AA-clone tables and training metadata;
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Evaluate IGH public CDR3-AA threshold stability on training data.",
...
def build_markdown_summary(
    ...
    lines: List[str] = [
        "# 03 IGH Public CDR3-AA Threshold Stability Summary",
        "",
        "## Scope",
        "",
...
def main() -> int:
    ...
    print("")
    print("=" * 80)
    print("03A IGH Public CDR3-AA 阈值稳定性评估")
    print("=" * 80)
    ...
    print("注意：脚本只读取固定IGH训练集，不读取test目录，也不计算模型AUC。")
    print("")
