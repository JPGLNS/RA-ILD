...
def parse_args():
    p = argparse.ArgumentParser(
        description="Build step-04 IGH train/test feature matrices.",
    ...
    p.add_argument("--expected-train-samples", type=int, default=120)
    p.add_argument("--expected-test-samples", type=int, default=49)
...
def group_of(n: str):
    ...
    if n in QC: return "igh_qc_depth"
    if n in DIVERSITY: return "igh_diversity"
    if n in EXPANSION: return "igh_clonal_expansion"
    if n.startswith("unweighted_3mer_"): return "igh_3mer_unweighted"
    if n.startswith("weighted_3mer_"): return "igh_3mer_weighted"
    ...
    if "aa_length" in n or n.startswith("weighted_length_"): return "igh_aa_length"
    if n.endswith("_frequency"):
        core = n.removeprefix("weighted_").split("_")[0]
        if len(core)==1:
            return "igh_aa_composition_weighted" if n.startswith("weighted_") else "igh_aa_composition_unweighted"
    if any(t in n for t in ("hydrophobicity","charge","aromaticity",
                            "basic_aa_ratio","acidic_aa_ratio","polar_aa_ratio","nonpolar_aa_ratio")):
        return "igh_physicochemical"
    return "igh_other_static"
...
def role_of(n, g):
    ...
    if g=="igh_qc_depth": return "qc_or_sensitivity_predictor"
    ...
    if g.startswith("igh_"): return "candidate_predictor"
    return "review"
...
def write_summary(path, title, inputs, outputs, shapes, runtime):
    lines = [f"# {title}","",f"- Script version: `{VERSION}`", f"- Receptor: `{RECEPTOR}`",f"- Runtime: `{runtime:.2f} seconds`","",
             "## Inputs",""]
...
def main():
    ...
    manifest.insert(0, "receptor", RECEPTOR)
    ...
    print("="*80)
    print("04 IGH Final Feature Matrix Build")
    print("="*80)
