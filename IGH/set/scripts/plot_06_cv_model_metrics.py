...
"""Visualize and compare repeated nested-CV performance for RA vs RA-ILD IGH models.
...
MODEL_LABELS = {
    "M0_clinical": "M0\nClinical",
    "M1_static_igh": "M1\nClinical + static IGH",
    "M2_static_igh_public": "M2\n+ dynamic public",
    "M3_static_igh_public_material": "M3\n+ material",
}

MODEL_COLORS = {
    "M0_clinical": "#65c8cc",
    "M1_static_igh": "#f0e94b",
    "M2_static_igh_public": "#72c15a",
    "M3_static_igh_public_material": "#f3793b",
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Plot and statistically compare IGH repeated nested-CV model metrics.",
...
def validate_predictions(
    ...
    if out.duplicated(["outer_repeat", "model", "sample_id"]).any():
        raise ValueError(
            "A sample has duplicate OOF predictions within a repeat and model."
        )
    ...
    if out["probability_ILD"].isna().any():
        raise ValueError("probability_ILD contains missing values.")
    if not np.isfinite(out["probability_ILD"].to_numpy()).all():
        raise ValueError("probability_ILD contains non-finite values.")
    ...
    expected_prediction_rows = (
        expected_repeats * expected_samples * len(MODELS)
    )
    ...
    if not bad_counts.empty:
        raise ValueError(
            ...
            f"{expected_samples} unique OOF samples: {bad_counts.to_dict()}"
        )

    per_task = out.groupby(["outer_repeat", "outer_fold", "model"])["sample_id"].nunique()
    expected_per_fold = expected_samples / expected_folds
    if not expected_per_fold.is_integer():
        raise ValueError(
            "Expected samples are not divisible by expected folds; "
            "a fixed per-fold validation size cannot be asserted."
        )
    expected_per_fold = int(expected_per_fold)
    bad_task_counts = per_task[per_task != expected_per_fold]
    if not bad_task_counts.empty:
        raise ValueError(
            f"Each outer task/model must contain {expected_per_fold} predictions: "
            f"{bad_task_counts.to_dict()}"
        )

    label_consistency = out.groupby(
        ["outer_repeat", "sample_id"]
    )["true_label"].nunique()
    if (label_consistency != 1).any():
        raise ValueError("A sample has inconsistent true labels across models.")

    sample_sets = {
        (repeat, model): set(group["sample_id"])
    ...
    for key, sample_set in sample_sets.items():
        if sample_set != reference_set:
            raise ValueError(
                f"OOF sample set for repeat/model {key} differs from the fixed cohort."
            )
...
def plot_metric_panels(
    ...
    figure.suptitle(
        "Repeated Nested-CV Performance of RA vs RA-ILD IGH Models",
        fontsize=17,
        fontweight="bold",
        y=0.99,
    )
...
def main() -> int:
    ...
    print("=" * 80)
    print("06 IGH Repeated-CV Model Metric Visualization")
    print("=" * 80)
    ...
    print(
        ...
        f"{args.expected_samples} unique OOF samples"
    )
    print("- PASS: paired Friedman/Wilcoxon tests use repeat-level OOF metrics")
    print("[Output files]")
