#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Finalize Batch 15 installation after the real-data smoke workflow passes."""
...
from ra_ild_igh.xgboost_config import load_xgboost_config  # noqa: E402

ACCEPTANCE = "IGH_BATCH15_XGBOOST_ACCEPTANCE_PASS"


def main() -> int:
    parser = argparse.ArgumentParser(description="Validate the Batch 15 real-data smoke outputs and finalize the installation marker.")
    parser.add_argument("--repository-root", required=True)
    parser.add_argument(
        "--config",
        default="IGH/set/experiments/igh_xgboost_smoke_m0_v1/00_config/resolved_config.yaml",
    )
    ...
    try:
        ...
        if config.experiment_id != "igh_xgboost_smoke_m0_v1":
            raise ValueError(f"Acceptance requires smoke config igh_xgboost_smoke_m0_v1; observed={config.experiment_id}")
        ...
        install_path = root / "IGH/set/scripts_v2/BATCH15_XGBOOST_INSTALLATION.json"
        if not install_path.is_file():
            raise FileNotFoundError("Batch 15 installation marker missing")
        ...
        print("IGH Batch 15 XGBoost real-data smoke acceptance: PASS")
        ...
        print(f"IGH Batch 15 XGBoost acceptance: FAIL\n{exc}", file=sys.stderr)
        return 1
