...
class Batch04RepeatedHoldoutTrainingTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        for relative in (
            "IGH/set/train/result/04_final_feature_matrix",
            "IGH/set/test/result/04_final_feature_matrix",
            "IGH/set/train/result/01_AA_clone_table",
            "IGH/set/test/result/01_AA_clone_table",
            "IGH/set/split_sets/test_repeat2",
            "IGH/set/configs/repeated_holdout_training",
            ...
            rows.append(
                {
                    ...
                    "age": 35 + index,
                    "aa_clone_number": 2,
                    "igh_feature_1": float(index),
                    "igh_feature_2": float(index % 4),
                    "all_ref_public_frequency_sum": 999.0,
                }
            )
        full = pd.DataFrame(rows)
        full.iloc[:21].to_csv(
            self.root / "IGH/set/train/result/04_final_feature_matrix/04_train_base_feature_matrix.csv",
            index=False,
        )
        full.iloc[21:].to_csv(
            self.root / "IGH/set/test/result/04_final_feature_matrix/04_test_final_feature_matrix.csv",
            index=False,
        )
        pd.DataFrame(
            {
                "feature_name": ["igh_feature_1", "igh_feature_2", "ignored"],
                "feature_group": ["igh_test", "igh_test", "qc"],
            ...
            self.root / "IGH/set/train/result/04_final_feature_matrix/04_feature_manifest.csv",
            index=False,
        )
        for index, sample_id in enumerate(self.sample_ids):
            directory = (
                self.root / "IGH/set/train/result/01_AA_clone_table"
                if index < 21
                else self.root / "IGH/set/test/result/01_AA_clone_table"
            )
            pd.DataFrame(
                {
        ...
        self.assignments_path = self.root / "IGH/set/split_sets/test_repeat2/repeated_holdout_assignments.csv"
        ...
        (self.root / "IGH/set/split_sets/test_repeat2/SPLITS_FROZEN.json").write_text(
        ...
        template = {
            ...
            "scheme": {
                ...
                "status": "development",
                "base_config": "IGH/set/configs/trb_baseline_m2_v1.yaml",
                "output_root": "IGH/set/experiments/template_scheme",
            },
            "replacements": {
                "models": {
                    "M1": {
        ...
        (self.root / "IGH/set/configs/schemes/template.yaml").write_text(
            yaml.safe_dump(template, sort_keys=False), encoding="utf-8"
        )
        self.spec_path = self.root / "IGH/set/configs/repeated_holdout_training/test.yaml"
            ...
            yaml.safe_dump(
                {
                    ...
                    "bundle": {
                        "id": "test_bundle",
                        "output_root": "IGH/set/training_bundles/test_bundle",
                    },
                    "split_set": {
                        "assignments": "IGH/set/split_sets/test_repeat2/repeated_holdout_assignments.csv",
                        "frozen_marker": "IGH/set/split_sets/test_repeat2/SPLITS_FROZEN.json",
                        "train_role": "train",
                    ...
                    "scheme": {
                        "template": "IGH/set/configs/schemes/template.yaml",
                        "generated_scheme": "IGH/set/configs/schemes/generated/generated.yaml",
                        "id": "generated_scheme",
                        "output_root": "IGH/set/experiments/generated_scheme",
                    },
                },
                sort_keys=False,
            ),
    ...
    def test_full_static_matrix_preserves_frozen_order_and_excludes_old_public(self) -> None:
        ...
        self.assertEqual(static, ("igh_feature_1", "igh_feature_2"))
    ...
    def test_missing_aa_table_is_rejected(self) -> None:
        (self.root / "IGH/set/test/result/01_AA_clone_table/S29_AA_clone_table.csv").unlink()
