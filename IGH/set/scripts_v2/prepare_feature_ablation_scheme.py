...
from ra_ild_igh.config import validate_experiment_mapping  # noqa: E402
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Prepare a IGH repeat-specific 3-mer feature-ablation scheme.",
...
def _group_count(group: str, repeat_options: Mapping[str, Any], legacy_count: int) -> int:
    ...
    legacy_name = str(
        repeat_options.get("legacy_group_name", "static_igh_candidate_predictors")
    )
...
def prepare(source_path: Path, repository_root: Path, *, overwrite: bool) -> Path:
    ...
    allowed_root = (repository_root / "IGH/set/experiments").resolve()
    try:
    ...
    repeat_options.setdefault("legacy_group_name", "static_igh_candidate_predictors")
    ...
    if not isinstance(source_selection, Mapping):
        raise FeatureAblationSchemeError(
            "root.model_selection must be a mapping"
        )
    ...
    sort_columns, sort_ascending = tuning_sort_policy(
        tuning_primary_metric
    )
    ...
    resolved_selection["tuning_primary_metric"] = (
        tuning_primary_metric
    )
...
def main() -> int:
    ...
    try:
        ...
        print("IGH feature-ablation scheme preparation: COMPLETE")
        print(f"Resolved config: {resolved}")
        return 0
    except Exception as exc:
        print(f"IGH feature-ablation scheme preparation: FAIL\n{exc}", file=sys.stderr)
        return 1
