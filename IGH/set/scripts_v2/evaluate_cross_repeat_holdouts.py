#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Run frozen-model n×n cross-repeat holdout evaluation for IGH."""
...
from ra_ild_igh.feature_inputs import load_partition_feature_matrix
from ra_ild_igh.nested_cv import NestedCVOptions
from ra_ild_igh.outer_cv import build_outer_tasks, scan_outer_tasks, task_output_paths
from ra_ild_igh.public_reference import ThresholdScheme
from ra_ild_igh.repeated_holdout_summary import load_frozen_assignments
from ra_ild_igh.specifications import (
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate every repeat-specific final IGH model i on every frozen "
...
def read_csv(path: Path, label: str) -> pd.DataFrame:
    ...
    if duplicated:
        raise ValueError(f"{label} duplicated columns: {duplicated[:20]}")
    return frame
...
def load_inputs(config):
    ...
    if base["sample_id"].duplicated().any() or base["sample_id"].tolist() != sample_ids:
        raise ValueError(
            "Sparse-cache row order must match unique base-matrix sample order"
        )
...
def load_task_artifacts(
    ...
    for repeat, directory in sorted(task_dirs.items()):
        ...
        configuration = json.loads(
            paths["configuration"].read_text(encoding="utf-8")
        )
        if int(configuration.get("outer_repeat", -1)) != int(repeat):
            raise ValueError(
                f"Task configuration repeat mismatch for repeat {repeat}"
            )
        ...
        result[int(repeat)] = FrozenTaskArtifacts(
            ...
            coefficients=read_csv(
                paths["coefficients"],
                f"repeat {repeat} coefficients",
            ),
            preprocessing=read_csv(
                paths["preprocessing"],
                f"repeat {repeat} preprocessing",
            ),
            outer_train_public=read_csv(
                paths["outer_train_public"],
                f"repeat {repeat} outer-train public",
            ),
            outer_validation_public=read_csv(
                paths["outer_validation_public"],
                f"repeat {repeat} outer-validation public",
            ),
            native_predictions=read_csv(
                paths["outer_predictions"],
                f"repeat {repeat} native predictions",
            ),
        )
    return MappingProxyType(result)
...
def main() -> int:
    ...
    try:
        ...
        if str(repeated.get("mode")) != "frozen_repeated_holdout":
            raise ValueError(
                "Requires repeated_holdout_training.mode=frozen_repeated_holdout"
            )
        ...
        if len(executed_folds) != 1:
            raise ValueError(
                "Exactly one outer fold must be executed per repeated holdout"
            )
        ...
        tasks = tuple(
            task for task in all_tasks if task.outer_fold == executed_fold
        )
        if len(tasks) != repeats:
            raise ValueError(
                f"Executed tasks={len(tasks)}, expected repeats={repeats}"
            )
        ...
        if not status["status"].eq("complete").all():
            ...
            raise ValueError(
                "All repeated-holdout tasks must be complete:\n"
        ...
        model_specs = resolve_all_model_specifications(
            ...
            static_group_name=str(
                config.raw["model_selection"]["static_feature_group"]
            ),
        )
        ...
        aa_clone_numbers = pd.to_numeric(
            base["aa_clone_number"],
            errors="raise",
        ).to_numpy(float)
        threshold_scheme = ThresholdScheme.from_mapping(public)

        assignments, _, assignment_sha256, _ = load_frozen_assignments(
            config.path(
                "repeated_holdout_training.assignments",
                must_exist=True,
                expect="file",
            ),
            config.path(
                "repeated_holdout_training.frozen_marker",
                must_exist=True,
                expect="file",
            ),
        ...
        expected_assignment_rows = (
            int(config.raw["data"]["train"]["expected_samples"]) * repeats
        )
        if len(assignments) != expected_assignment_rows:
            raise ValueError(
                f"Frozen assignment rows={len(assignments)}, "
                f"expected={expected_assignment_rows}"
            )
        ...
        output_dir = (
            ...
            if args.output_dir
            else config.path("aggregation.output_dir").parent
        ...
        print("IGH frozen-model cross-repeat evaluation: COMPLETE")
        print(f"Repeats:              {repeats}")
        print(f"Selected models:      {len(models)}")
        print(
            f"Pairs per model:      {repeats} x {repeats} "
            f"= {repeats * repeats}"
        )
        ...
        print(
            f"IGH frozen-model cross-repeat evaluation: FAIL\n{exc}",
            file=sys.stderr,
        )
        return 1
