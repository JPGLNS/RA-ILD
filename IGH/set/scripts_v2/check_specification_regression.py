...
def main() -> int:
    ...
    try:
        ...
        selected_static = select_static_igh_features(
            manifest,
        ...
        dynamic_models_ok = (
            list(resolved["M2_static_igh_public"].numeric[-len(public_features):])
            == public_features
            and list(
                resolved["M3_static_igh_public_material"].numeric[-len(public_features):]
            )
            == public_features
        )
        add(
