# Batch 09：YAML参数网格与Ridge/Lasso端点升级
...
```bash
python IGH/set/scripts_v2/test_batch09_alpha_grid_ridge.py
```
...
Batch 09 focused tests: 3/3 passed
```
...
python IGH/set/scripts_v2/prepare_feature_ablation_scheme.py \
  --scheme IGH/set/configs/schemes/<你的参数方案>.yaml
```
...
python IGH/set/scripts_v2/run_all_outer_tasks.py \
  --config IGH/set/experiments/<experiment_id>/00_config/resolved_config.yaml \
  --status-only
